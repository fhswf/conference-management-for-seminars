...
function StudentDetailPage() {
    ...
    const {data} = useFetch<UserType>(`http://${import.meta.env.VITE_BACKEND_URL}/api/seminar/get-student/${seminarOID}/${studentOID}`);
    ...
    const tableData = data?.userOIDStudent_concepts.map((concept) => {
        return {
            ...
            pdf: concept.attachmentO ?
                <a href={`http://${import.meta.env.VITE_BACKEND_URL}/api/attachment/${concept.attachmentO?.attachmentOID}`}>{concept.attachmentO?.filename}</a> : "-",
