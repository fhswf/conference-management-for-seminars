...
describe('SeminarPage', () => {
    ...
    beforeEach(function () {
        ...
        cy.intercept('GET', `${Cypress.env('VITE_BACKEND_PROTOCOL')}://${Cypress.env('VITE_BACKEND_URL')}/paper/get-assigned-paper/${seminarOID}`, {
            statusCode: 200,
    ...
    describe('concept part for student', function () {
        ...
        it('test concept upload process if button enabled', function () {
            if (!isJsonEmpty(this.concept) && (this.concept.accepted === null || this.concept.accepted || this.seminar.phase !== 2) && this.concept.accepted !== false) {
                ...
                cy.intercept('POST', `${Cypress.env('VITE_BACKEND_PROTOCOL')}://${Cypress.env('VITE_BACKEND_URL')}/concepts`, (req) => {
                    req.reply({
                        statusCode: 200,
            ...
            it('click on comment button should open chat modal, and should display messages correctly', function () {
                cy.getByData('assigned-papers-div')
                    ...
                    .each((paperRow, index) => {
                        ...
                        cy.intercept('GET', `${Cypress.env('VITE_BACKEND_PROTOCOL')}://${Cypress.env('VITE_BACKEND_URL')}/chat/${this.assignedPaper[index].reviews[0].reviewOID}`, {
                            statusCode: 200,
                            fixture: 'chatMessages.json',
                        }).as('getChatMessages');
